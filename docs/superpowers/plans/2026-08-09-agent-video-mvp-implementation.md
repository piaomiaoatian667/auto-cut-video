...
`videoctl review <project> --approve --reason <text>` resolves the work pointer, then calls `RunStore.openExistingRun(projectId, runId)` and uses the returned opaque `RunDirectoryScope` to verify every evidence path, writes strict `review.json`, and never edits programmatic checks. A rejected review records `status: rejected` and blocks Release.
